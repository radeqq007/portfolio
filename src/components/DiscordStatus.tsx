...
const DiscordStatus = ({ userID, avatar, globalName, status, song, artist, isListening, activities = [] }: Props) => {
  ...
  if (activity) {
    activityContent = (
      <div className="text-text-secondary text-lg flex flex-col gap-0.5">
        ...
        {activity.details && <span className="text-sm">{activity.details}</span>}
    ...
    activityContent = (
      <span className="text-text-secondary text-sm">
        Listening to <span className="text-text font-medium">{song}</span> by <span className="text-text font-medium">{artist}</span>
      </span>
    );
    ...
    activityContent = <span className="text-text-secondary text-sm">No activity</span>;
  }
